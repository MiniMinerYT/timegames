...
Each run starts with three lives. Before every round, the target is clearly shown. Starting the round triggers the standard 3-second countdown, then the player stops a hidden timer. A successful stop adds one point; a miss removes one life. The run ends at zero lives.
...
- GOD: displayed error must be exactly 0.00 seconds; hidden until Expert reaches at least 3

Locked Medium, Hard and Expert cards remain visible and explain their unlock requirement. GOD is completely hidden until unlocked. Best scores persist independently for every difficulty.
...
Clock Rating is unaffected by Casual Single Player, Party Mode, Daily Challenge, Daily practice, Time Ladder and Hardcore Mode.
...
Stats reset clears Time Guesser accuracy stats and Clock Rating after confirmation. It does not clear Daily history, Daily practice bests, Time Ladder best or Hardcore best scores. Streaks are not used.
...
- Keep Time Ladder, Hardcore, Party, Daily and casual play isolated from Clock Rating.
